...
def _train(args):
    ...
    cnn_curve, nme_curve = {"top1": [], "top3": []}, {"top1": [], "top3": []}
    ...
    for task in range(data_manager.nb_tasks):
        ...
        # if task == 1:
        #     # 使用测试集进行可视化
        #     # 注意：需要确保 model 中有 test_loader，icarl.py 中已经赋值了 self.test_loader
        #     if hasattr(model, 'test_loader'):
        #         model.plot_tsne(model.test_loader, task)
        #     else:
        #         # 如果 model 没有保存 loader，重新从 data_manager 获取
        #         # test_dataset = data_manager.get_dataset(np.arange(0, model._total_classes), source="test", mode="test")
        #         # test_loader = DataLoader(test_dataset, batch_size=128, shuffle=False, num_workers=4)
        #         # model.plot_tsne(test_loader, task)
        #         pass
        # elif task == 2:
        #     if hasattr(model, 'test_loader'):
        #         model.plot_tsne(model.test_loader, task)
